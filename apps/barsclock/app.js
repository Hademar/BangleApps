...
let SETTINGS_FILE = "barsclock.json";
let settings = Object.assign(
    storage.readJSON("barsclock.default.json", true) || {},
...
let screenHeight = g.getHeight() - widgetOffset;
let screenWidth = g.getWidth();
let hOffset = (circleCount == 3 ? 34 : 30) - widgetOffset;
let h1 = Math.round(1 * screenHeight / 5 - hOffset);
let h2 = Math.round(3 * screenHeight / 5 - hOffset);
let h3 = Math.round(8 * screenHeight / 8 - hOffset - 3); // circle y position
...
let circlePosX = [
  Math.round(1 * screenWidth / parts), // circle1
  Math.round(3 * screenWidth / parts), // circle2
  Math.round(5 * screenWidth / parts), // circle3
  Math.round(7 * screenWidth / parts), // circle4
];
...
function draw() {
  ...
  if (showBigWeather) {
    ...
    g.setFontAlign(1, 0);
    if (tempString) g.drawString(tempString, screenWidth, h2);

    let code = weather ? weather.code : -1;
    let icon = getWeatherIconByCode(code, true);
    if (icon) g.drawImage(icon, screenWidth - 48, h1, {scale:0.75});
  }
